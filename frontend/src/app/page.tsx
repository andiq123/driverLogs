      ...
      <AppShell view={app.view} userName={app.settings.name} onLogout={app.logout} onViewChange={app.setView}>
        <AnimatePresence mode="wait">
          <motion.div key={app.view} {...softReveal} className="pt-4">
            ...
            {!app.isLoadingData && app.view === "Dashboard" && <DashboardView vehicle={app.activeVehicle} totals={app.vehicleTotals} token={app.token} baseCurrency={app.settings.default_currency} country={app.settings.country} savingExpense={app.action === "expense"} onCreateExpense={app.saveExpense} />}
            {!app.isLoadingData && app.view === "Timeline" && <TimelineView expenses={app.activeExpenses} vehicle={app.activeVehicle} />}
