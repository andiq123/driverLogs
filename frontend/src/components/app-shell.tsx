...
export function AppShell({ view, userName, onLogout, onViewChange, children }: { view: View; userName?: string; onLogout: () => void; onViewChange: (view: View) => void; children: ReactNode }) {
  ...
  return (
    <main className="min-h-dvh bg-[#f5f7f2] text-[#151712]">
      <div className="mx-auto flex min-h-dvh w-full max-w-[96rem] gap-4 px-2 pb-[calc(4.75rem+env(safe-area-inset-bottom))] pt-[max(0.5rem,env(safe-area-inset-top))] sm:px-5 lg:px-6 lg:py-3 xl:gap-6">
          ...
          <header className="sticky top-0 z-20 -mx-2 border-b border-black/[0.06] bg-[#f5f7f2]/88 px-3 pb-2 pt-[max(0.5rem,env(safe-area-inset-top))] backdrop-blur-xl sm:-mx-5 sm:px-5 sm:pb-3 sm:pt-[max(0.75rem,env(safe-area-inset-top))] lg:relative lg:mx-0 lg:border-none lg:bg-transparent lg:px-0 lg:pt-0 lg:backdrop-blur-none">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#70776a] sm:text-xs">Monday, June 1</p>
                <h1 className="mt-0.5 truncate text-[30px] font-semibold leading-none tracking-tight sm:mt-1 sm:text-4xl">{title}</h1>
              </div>
            </div>
          </header>
          {children}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-black/[0.08] bg-[#fbfcf8]/94 px-2 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-xl lg:hidden">
        <div className="mx-auto grid max-w-md grid-cols-5 gap-1">
          {mobileNavItems.map((item) => (
            <button key={item.label} aria-label={item.label} onClick={() => onViewChange(item.label)} className={`flex h-12 touch-manipulation flex-col items-center justify-center gap-1 rounded-[16px] text-[11px] font-semibold transition-[background-color,color,transform] duration-200 active:scale-[0.985] ${view === item.label ? "bg-[#e6f0df] text-[#151712]" : "text-[#70776a]"}`}>
              <item.icon size={18} />
