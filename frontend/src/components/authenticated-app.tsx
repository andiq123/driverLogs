    ...
    <>
      ...
      >
        <motion.div key={app.view} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.28, ease: [0.32, 0.72, 0, 1] }} className="min-h-[calc(100dvh-9rem)] pt-3 sm:pt-4">
          ...
          {!app.isLoadingData && app.view === "Dashboard" && <DashboardView vehicle={app.activeVehicle} expenses={app.activeExpenses} trips={app.activeTrips} userDocuments={app.userDocuments} totals={app.vehicleTotals} token={app.token} baseCurrency={app.settings.default_currency} country={app.settings.country} savingExpense={app.action === "expense"} savingTrip={app.action === "trip"} isDemo={app.isDemo} onCreateExpense={app.saveExpense} onStartTrip={app.startVehicleTrip} onEndTrip={app.endVehicleTrip} onReviewUnusualRecords={app.reviewUnusualRecords} />}
          {!app.isLoadingData && app.view === "Timeline" && <TimelineView expenses={app.activeExpenses} trips={app.activeTrips} vehicle={app.activeVehicle} token={app.token} baseCurrency={app.settings.default_currency} country={app.settings.country} savingExpense={app.action === "expense" || app.action === "delete"} openExpenseFilesID={app.openExpenseFilesID} onOpenExpenseFiles={app.setOpenExpenseFilesID} onDeleteExpense={app.removeExpense} onUpdateExpense={app.editExpense} onToggleAnalytics={app.toggleExpenseAnalytics} reviewAnomalies={app.reviewAnomalies} onClearUnusualRecordReview={app.clearUnusualRecordReview} />}
          {!app.isLoadingData && app.view === "Analytics" && <AnalyticsView mounted={app.mounted} vehicle={app.activeVehicle} totals={app.vehicleTotals} exporting={app.exportingReport} onExport={app.exportReport} />}
