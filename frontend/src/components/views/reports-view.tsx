...
export function ReportsView({ vehicle, totals }: { vehicle?: Vehicle; totals: MoneyTotals }) {
  if (!vehicle) {
  ...
  return (
    <div className="grid gap-3 sm:gap-4 xl:grid-cols-3">
      ...
      <ReportCard title="Expenses" value={String(totals.expense_count)} label="included records" />
      ...
      <section className="rounded-[24px] border border-black/[0.06] bg-[#fbfcf8] p-4 shadow-[0_14px_48px_rgba(31,41,28,0.08)] sm:rounded-[28px] sm:p-5 xl:col-span-3">
        {totals.expense_count === 0 ? <EmptyState icon={ReceiptText} title="No included expenses yet" body="Include at least one expense in analytics to build this report." /> : (
          <>
